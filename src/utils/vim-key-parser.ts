...
export function parseVimKey(input: string): ParsedVimKey {
	...
	if (trimmed.length === 1) {
		...
		// Lowercase or other single characters (including special chars)
		// Note: For special characters, the key will be the character itself
		// This works for most cases, but angle bracket notation is more reliable
		return {
			modifiers: [],
			key: char
		};
